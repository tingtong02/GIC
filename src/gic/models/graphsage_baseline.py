...
class GraphSAGEBaseline(BaselineModel):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, layers: int = 2, dropout: float = 0.0) -> None:
        super().__init__()
        if input_dim <= 0:
            raise ValueError('GraphSAGEBaseline input_dim must be positive')
        if layers < 1:
            raise ValueError('GraphSAGEBaseline requires at least one layer')
        self.dropout = nn.Dropout(float(dropout)) if dropout > 0 else nn.Identity()
        hidden_layers: list[nn.Module] = []
        previous_dim = input_dim
        for _ in range(max(1, layers - 1)):
            hidden_layers.append(GraphSAGELayer(previous_dim, hidden_dim))
            previous_dim = hidden_dim
        self.hidden_layers = nn.ModuleList(hidden_layers)
        self.output_layer = GraphSAGELayer(previous_dim, 1)
    ...
    def forward(
        ...
        *,
        ...
        if adjacency is None:
            raise ValueError('GraphSAGEBaseline requires adjacency information')
        hidden = features
...
def build_graphsage_baseline(model_config: dict[str, Any], input_dim: int) -> GraphSAGEBaseline:
